...
BASE_DIR: Path = Path(__file__).parent / "../workspaces"
BASE_DIR.mkdir(exist_ok=True)

active_connections: dict[str, WebSocket] = {}

def get_file_tree(session_dir: Path, current_path: Path) -> list[dict[str, str]]:
    tree: list[dict[str, str]] = []
    for item in current_path.iterdir():

        rel_path: str = item.relative_to(session_dir).as_posix()
        if item.is_dir():
            tree.append({
                "name": item.name,
                "path": rel_path,
...
async def read_file_content(session_dir: Path, file_path: str) -> str | None:
    full_path: Path = (session_dir / file_path).resolve()
...
async def write_file_content(session_dir: Path, file_path: str, content: str) -> None:
    full_path: Path = (session_dir / file_path).resolve()
    if not str(full_path).startswith(str(session_dir.resolve())):
...
async def filesystem_watcher(session_id: str, session_dir: Path) -> None:
    try:
...
async def broadcast_fs_update(session_id: str) -> None:
...
async def remove_connection(session_id: str) -> None:
    if session_id in active_connections:
        del active_connections[session_id]
        print(f"Removed connection {session_id}")

@app.post("/upload")
async def upload_project(file: UploadFile = File(...)) -> dict[str, str]:
    ...
    zip_path: Path = session_dir / "project.zip"
...
async def websocket_endpoint(websocket: WebSocket, session_id: str) -> None:
    ...
    safe_path: Path = (BASE_DIR / session_id).resolve()
    ...
    async def fs_update_callback() -> None:
        await broadcast_fs_update(session_id)

    agent: AgentSession = AgentSession(str(session_dir), send_log, fs_update_callback)
    ...
    try:
        while True:
            ...
            if data["type"] == "user_message":
                ...
                tree: list[dict[str, str]] = get_file_tree(session_dir, session_dir)
                await websocket.send_json({"type": "file_tree", "content": tree})

            elif data["type"] == "read_file":
                file_content: str | None = await read_file_content(session_dir, data["path"])
...
async def download_project(session_id: str) -> StreamingResponse:
    session_dir: Path = (BASE_DIR / session_id).resolve()
    ...
    zip_bytes: bytes = await asyncio.get_event_loop().run_in_executor(
        None, lambda: zip_workspace(session_dir)
    )

    filename: str = f"{session_id}.zip"
...
def zip_workspace(session_dir: Path) -> bytes:
    ...
    with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for p in session_dir.rglob("*"):
            ...
            rel: str = p.relative_to(session_dir).as_posix()
