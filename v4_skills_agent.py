...
def agent_loop(messages: list) -> list:
    while True:
        ...
        for tool_call in tool_calls:
            ...
            output = execute_tool(tool_name, function_args)
            if tool_name == "Skill":
                print(f"{Colors.RED}Skill loaded ({len(output)} chars){Colors.RESET}")
            elif tool_name != "Task":
                preview = output[:300] + "..." if len(output) > 300 else output
                print(f"{Colors.WHITE}{preview or '(empty)'}{Colors.RESET}")
            messages.append(
...
def main():
    ...
    message = Text.from_markup(f"Mini Claude Code v4 (with Skills) - [green]{WORKDIR}[/]\nCurrent Time: [green]{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}[/]\nSkills: [green]{', '.join(SKILLS.list_skills()) or 'none'}[/]\nAgent Types: [green]{', '.join(AGENT_TYPES.keys())}[/]")
